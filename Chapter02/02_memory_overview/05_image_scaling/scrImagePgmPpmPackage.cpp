...
void scr_write_pgm(char *name, unsigned char *image, int rows, int cols,
                   const char *comment) {
  int i, j;
  unsigned char *tpt;
  char fname[256];
...
void scr_write_ppm(char *name, unsigned char *image, int rows, int cols,
                   char *comment) {
  int i, j;
  unsigned char *tpt;
  char fname[256];
  ...
  for (i = 0; i < rows; ++i) {
    for (j = 0; j < cols; ++j, tpt += 3) {
      fprintf(fdout, "%c%c%c", (unsigned char)(*(tpt + 2)),
              (unsigned char)(*(tpt + 1)), (unsigned char)(*tpt));
    }
  }
  fprintf(fdout, "\n");
  fclose(fdout);
