...
__global__ void softmax_loss_kernel(float *reduced_loss, float *predict,
                                    float *target, float *workspace,
                                    int batch_size, int num_outputs) {
...
float CrossEntropyLoss::loss(Blob<float> *predict, Blob<float> *target) {
  ...
  cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &num_blocks_per_sm, softmax_loss_kernel, BLOCK_DIM_1D,
      BLOCK_DIM_1D * sizeof(float));

  int batch_size = target->n();
  int num_outputs = target->n();

  init_workspace(batch_size);

  int num_blocks = min(num_blocks_per_sm * num_sms,
                       (target->size() + BLOCK_DIM_1D - 1) / BLOCK_DIM_1D);
  softmax_loss_kernel<<<num_blocks, BLOCK_DIM_1D, BLOCK_DIM_1D * sizeof(float),
                        0>>>(d_loss_, predict->cuda(), target->cuda(),
                             d_workspace_, batch_size, num_outputs);
