...
void cudl::MNIST::create_shared_space() {
  data_ = new cudl::Blob<float>(batch_size_, channels_, height_, width_);
  data_->tensor();
  target_ = new cudl::Blob<float>(batch_size_, num_classes_);
}

void cudl::MNIST::load_data(std::string &image_file_path) {
  uint8_t ptr[4];
...
void cudl::MNIST::load_target(std::string &label_file_path) {
  uint8_t ptr[4];
...
void cudl::MNIST::shuffle_dataset() {
...
int cudl::MNIST::to_int(uint8_t *ptr) {
  return ((ptr[0] & 0xFF) << 24 | (ptr[1] & 0xFF) << 16 | (ptr[2] & 0xFF) << 8 |
          (ptr[3] & 0xFF) << 0);
}

void cudl::MNIST::train(int batch_size, bool shuffle) {
  if (batch_size < 1) {
...
void cudl::MNIST::test(int batch_size) {
  if (batch_size < 1) {
    std::cout << "batch size should be greater than 1." << std::endl;
    return;
  }
